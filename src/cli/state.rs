...
impl ProgressTracker {
    ...
    pub fn handle(&mut self, event: ProgressEvent) {
        match event {
            ...
            ProgressEvent::ChunkAdvanced { worker, index, written } => {
                self.ensure_index(index);
...
mod tests {
    ...
    fn start(total: u64) -> ProgressEvent {
        ProgressEvent::Start { total, chunk_size: 100 }
    }
    ...
    fn written_bytes_track_the_max_across_retries() {
        ...
        tracker.handle(ProgressEvent::ChunkAdvanced { worker: 0, index: 0, written: 60 });
        tracker.handle(ProgressEvent::ChunkAdvanced { worker: 0, index: 0, written: 30 });
    ...
    fn chunks_are_attributed_to_the_worker_that_worked_them() {
        ...
        tracker.handle(ProgressEvent::ChunkStarted { worker: 1, index: 3 });
        tracker.handle(ProgressEvent::ChunkAdvanced { worker: 1, index: 3, written: 90 });
    ...
    fn completion_marks_worker_idle_and_chunk_done() {
        ...
        tracker.handle(ProgressEvent::ChunkStarted { worker: 0, index: 2 });
        tracker.handle(ProgressEvent::ChunkAdvanced { worker: 0, index: 2, written: 100 });
        tracker.handle(ProgressEvent::ChunkComplete { worker: 0, index: 2 });
    ...
    fn retried_chunk_keeps_byte_max_from_both_workers() {
        ...
        tracker.handle(ProgressEvent::ChunkAdvanced { worker: 0, index: 4, written: 40 });
        tracker.handle(ProgressEvent::ChunkAdvanced { worker: 2, index: 4, written: 80 });
    ...
    fn planned_chunks_and_offsets_map_back_to_chunks() {
        let mut tracker = ProgressTracker::new(2);
        tracker.handle(ProgressEvent::Start { total: 1000, chunk_size: 100 });
